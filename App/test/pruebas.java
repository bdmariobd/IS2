...
public class pruebas {
	...
	@Test // Prueba de insertar un alumno, aseguramos que la nueva ID va ser la ultima en la BBDD, comprobamos tb el metodo
	// create ID del Dao
	public void test0() {
		TAlumno transfer = new TAlumno(0,Integer.toString(getRandomDNI())+"Z", "Paco", "Marhuenda" , 915439233, "junits@gmail.com", false, true);
		int idNueva = FactoriaDAO.getInstance().generateDAOAlumno().getID();
		assertEquals(FactoriaSA.getInstance().generateSAalumno().create(transfer),idNueva);
